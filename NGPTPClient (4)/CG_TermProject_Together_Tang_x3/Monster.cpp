...
void CMonster::SetMonsterRecvPosition(vector<CGameObject*>* _vpMonster, MonsterInfo* _MonsterInfo)
{
	auto iter = _vpMonster->begin();

	for (int i = 0; i < 10; i++)
	{
		dynamic_cast<CMonster*>(*iter)->SetPosition(_MonsterInfo[i].MonsterPos);
		iter++;
		//if (i == 9)
			//iter = _vpMonster.begin();
	}
}
