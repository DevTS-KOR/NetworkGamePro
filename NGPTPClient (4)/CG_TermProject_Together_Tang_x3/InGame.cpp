...
GLvoid CInGame::Initialize(CBitmapMgr* _pBitmapMgr)
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		//cout << CSceneMgr::GetInitInfo()->MonsterPos[i].fX << ", " << CSceneMgr::GetInitInfo()->MonsterPos[i].fY << ", " << CSceneMgr::GetInitInfo()->MonsterPos[i].fZ << endl;
	}
	cout << "초기값 인덱스 : " << CSceneMgr::GetInitInfo()->playerIndex << endl;
	//}
	//glPopMatrix();
	m_iBlendcubeAngle = 0;
	m_iClear = 0;
		...
		//{
			...
			////glPopMatrix();
				glPushMatrix();
				{
					/*glPushMatrix();
					{
						glTranslatef(dynamic_cast<CCharacter*>(m_pCharacter)->GetPosition2().fX,
							dynamic_cast<CCharacter*>(m_pCharacter)->GetPosition2().fY,
							dynamic_cast<CCharacter*>(m_pCharacter)->GetPosition2().fZ);
						m_pCharacter->Draw();
					}
					glPopMatrix();*/
					//glPushMatrix();
					...
					for (int i = 0; i < 10; i++)
					{
						dynamic_cast<CMonster*>(*iter)->SetPosition(strMonsterPos->monsters[i].MonsterPos);
						iter++;
						if(i == 9)
...
GLvoid CInGame::Keyboard(unsigned char key, int x, int y)
{
	...
	//CSceneMgr::GetInst()->SendInfo(dynamic_cast<CCharacter*>(m_pCharacter)->GetPosition());
...
GLvoid CInGame::Update(int value)
{
	...
	{
		iter->Update();
		cout << "총알 : " << iter->GetPos().fX << ", " << iter->GetPos().fY << ", " << iter->GetPos().fZ << endl;
		if (iter->GetPosDistance() > 1000 || iter->GetPosDistance() < -1000)
		{
			m_vBullet.erase(iter);
			iter = m_vBullet.begin();
		}
	...
	CSceneMgr::GetInst()->SendInfo(dynamic_cast<CCharacter*>(m_pCharacter)->GetPosition(), DYNAMIC(CCharacter*, m_pCharacter)->GetCameraRotate().fX);
	Recv();
...
GLvoid CInGame::Mouse(int button, int state, int x, int y)
{
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		//
		//cout << "총알위치아님 : " << sx << ", " << sy << ", " << sz << endl;
		//cout << "총알위치아님2 : " << m_View.fX << ", " << m_View.fY << ", " << m_View.fZ << endl;
		//플레이어 위치가 나온다...
...
GLvoid CInGame::MouseMotionFunc(int x, int y)
{
	...
	//cout << "마우스 : " << DYNAMIC(CCharacter*, m_pCharacter)->GetCameraRotate().fX << ", " << DYNAMIC(CCharacter*, m_pCharacter)->GetCameraRotate().fY << ", " << DYNAMIC(CCharacter*, m_pCharacter)->GetCameraRotate().fZ << endl;
...
GLvoid CInGame::Recv()
{
	...
	PlayerInfo				tempInfo;
	DataPacket				tempDataPacket;

	retval = recv(CSceneMgr::GetInst()->GetSocket(), (char*)&dataType, sizeof(int), 0);
	if (retval == SOCKET_ERROR)
		err_quit("recv()");

	//cout << "무슨 데이터 타입 : " << dataType << endl;

	if (dataType == DataType::PACKET)
	{
		retval = recv(CSceneMgr::GetInst()->GetSocket(), (char*)&tempDataPacket, sizeof(DataPacket), 0);
		if (retval == SOCKET_ERROR)
		{
			err_display("send1()");
		}

		if (CSceneMgr::GetInitInfo()->playerIndex != tempDataPacket.player1.playerIndex)
		{
			dynamic_cast<CCharacter*>(m_pCharacter)->SetCharRecvPosition(CSceneMgr::GetInst()->GetInitInfo(), tempDataPacket.player1);
			//dynamic_cast<CCharacter*>(m_pCharacter)->SetCameraRecv(tempDataPacket.player1.CameraDir);
			//dynamic_cast<CCharacter*>(m_pCharacter)->SetCameraRecv();
		}
		else if (CSceneMgr::GetInitInfo()->playerIndex != tempDataPacket.player2.playerIndex)
		{
			dynamic_cast<CCharacter*>(m_pCharacter)->SetCharRecvPosition(CSceneMgr::GetInst()->GetInitInfo(), tempDataPacket.player2);
			//dynamic_cast<CCharacter*>(m_pCharacter)->SetCameraRecv(tempDataPacket.player2.CameraDir);
			//dynamic_cast<CCharacter*>(m_pCharacter)->SetCameraRecv();
		}

		dynamic_cast<CMonster*>(m_pMonster)->SetMonsterRecvPosition(&m_vpMonster, tempDataPacket.monsters);
	}

	/*if (dataType == DataType::PLAYER)
	{
		retval = recv(CSceneMgr::GetInst()->GetSocket(), (char*)&tempInfo, sizeof(PlayerInfo), 0);
		if (retval == SOCKET_ERROR)
			err_quit("recv()");
		...
		dynamic_cast<CCharacter*>(m_pCharacter)->SetCharRecvPosition(CSceneMgr::GetInst()->GetInitInfo(), strPlayerInfo);
	}

	if (dataType == DataType::MONSTER)
	{
		//cout << "니가1" << endl;
		//cout << "몬스터 받음" << endl;

		retval = recv(CSceneMgr::GetInst()->GetSocket(), (char*)&tempMonsterPos, sizeof(MonsterPosForRecv), 0);
		if (retval == SOCKET_ERROR)
			err_quit("recv()");

		strMonsterPos = &tempMonsterPos;
		dynamic_cast<CMonster*>(m_pMonster)->SetMonsterRecvPosition(&m_vpMonster, strMonsterPos);
		//cout << "니가2" << endl << endl;
	}*/

	/*if (dataType == DataType::NULLDATA)
	{
		char buf[BUFSIZE];
		cout << "널 값 받음" << endl;
		retval = recv(CSceneMgr::GetInst()->GetSocket(), (char*)&buf, sizeof(int), 0);
		if (retval == SOCKET_ERROR)
			err_quit("recv()");
	}*/
	...
	{
		char buf[BUFSIZE];
		retval = recv(CSceneMgr::GetInst()->GetSocket(), (char*)&buf, 0, 0);
		if (retval == SOCKET_ERROR)
			err_quit("recv()");
		//cout << "똥" << endl;
	}*/
...
void CInGame::err_quit(char * msg)
{
	LPVOID lpMsgBuf;
	FormatMessage(
...
void CInGame::err_display(char * msg)
{
	LPVOID lpMsgBuf;
	FormatMessage(
		FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
